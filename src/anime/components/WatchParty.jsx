...
export default function WatchParty({ route, media }) {
  ...
  const createRoom = async () => {
    try {
      ...
      const titleStr = media?.title_english || media?.title || (route.type === 'movie' ? `Movie ${route.id}` : route.type === 'tv' ? `TV Show ${route.id}` : `Anime ${route.id}`);
      await setDoc(doc(db, 'watchParties', newId), {
        type: route.type === undefined ? 'anime' : route.type,
        malId: route.id, // For anime
        tmdbId: route.id, // Fallback for tv/movie
        season: route.season || 1,
        episode: route.episode || 1,
        title: titleStr,
        posterPath: media?.poster_path || media?.images?.jpg?.large_image_url || '',
        startedAt: Date.now(),
  ...
  const isSyncMedia = roomData 
    ? (roomData.type === 'anime' ? roomData.malId === route.id || roomData.tmdbId === route.id : (roomData.tmdbId === route.id && roomData.type === route.type && (roomData.type === 'movie' || (roomData.season === route.season && roomData.episode === route.episode))))
    : true;

  const jumpToRoomMedia = () => {
    if (roomData.type === 'movie') {
      window.location.hash = `/play/movie/${roomData.tmdbId}?room=${roomId}`;
    } else if (roomData.type === 'tv') {
      window.location.hash = `/play/tv/${roomData.tmdbId}/${roomData.season}/${roomData.episode}?room=${roomId}`;
    } else {
      window.location.hash = `/play/anime/${roomData.malId || roomData.tmdbId}?room=${roomId}`;
    }
  };
    ...
    <>
      <div style={{ position: 'relative', zIndex: 10002 }}>
        {!isOpen ? (
          ...
          <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, background: 'var(--color-neo-surface)', padding: 20, borderRadius: 12, border: '1px solid var(--color-neo-border)', width: 300, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>

...
import { getDetails, logoUrl } from '../api/jikan.js';
...
export default function PlayerPage({ route, navigate }) {
  ...
  useEffect(() => {
    getDetails(route.id).then((data) => {
      setMedia(data);
      // Jikan doesn't provide isolated logos like TMDB, use the poster for watermark
      setLogo(logoUrl(data?.images));
    }).catch(console.error);
  }, [route.id]);

    ...
    <header className={`movie-header ${scrolled ? 'scrolled' : ''}`}>
      <div className="layout-container">
        <div className="flex items-center justify-between h-[68px] md:h-[80px]">
          ...
          >
            <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-primary flex items-center justify-center transition-transform duration-300 group-hover:scale-105">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polygon points="5,3 19,12 5,21" fill="white" />
              </svg>
            </div>
            <span className="text-xl md:text-2xl font-bold text-white">Kiruu</span>
          </a>
            ...
            <div className="relative">
              ...
              {browseOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 rounded-xl glass-card-dark p-2 shadow-xl" onClick={(e) => e.stopPropagation()}>
                  {[
                    { label: 'Popular Movies', path: '/browse/movie' },
                    { label: 'Popular TV Shows', path: '/browse/tv' },
                    { label: 'Top Rated Movies', path: '/genre/movie/top_rated' },
                    { label: 'Now Playing', path: '/genre/movie/now_playing' },
                  ].map((item) => (
                    <button
                      key={item.path}

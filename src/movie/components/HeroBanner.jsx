...
import { getTrending, backdropUrl } from '../api/tmdb.js';
...
export default function HeroBanner({ navigate }) {
  ...
  const genres = (item.genre_ids || []).slice(0, 2).map(getGenreName).filter(Boolean);
  ...
  return (
    <div className="hero-banner">
      {items.map((it, i) => (
        <img
          key={it.id}
          src={backdropUrl(it.backdrop_path)}
          ...
          <div style={{ maxWidth: 600 }}>
            ...
            <div className="flex items-center gap-3 fade-in-up" key={`cta-${current}`} style={{ animationDelay: '0.3s' }}>
              <button
                ...
                onClick={() => navigate(`/${type}/${item.id}/play`)}
              >
...
/* Simple genre ID → name lookup for common genres */
const GENRE_MAP = {
